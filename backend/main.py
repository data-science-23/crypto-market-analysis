...
class AnalysisPriceRequest(BaseModel):
    ticker: str
    timeframe: str = "recent"


class AnalysisSentimentRequest(BaseModel):
    ticker: str
    days: int = 7
...
def chat_with_bot(request: ChatRequest):
    """
    Chat with RAG-powered crypto assistant
    
    Request body:
    {
        "message": "What's the trend for BTC?",
        "search_collections": ["kline", "news"],  // optional
        "top_k": 5,  // optional
...
@app.post("/api/analyze/price-trend")
def analyze_price_trend_endpoint(request: AnalysisPriceRequest):
    """
    Get AI analysis of price trend for a ticker
    ...
    try:
        ...
        df = get_ohlcv_data(ticker=request.ticker, limit=100)
        ...
        analysis = rag_service.analyze_price_trend(
        ...
        return {
            ...
            "last_update": latest.get("Date", "").isoformat() if hasattr(latest.get("Date", ""), "isoformat") else str(latest.get("Date", ""))
        }
...
def analyze_sentiment_endpoint(request: AnalysisSentimentRequest):
    """
    Get AI analysis of news sentiment for a ticker
    ...
    """
    try:
        analysis = rag_service.analyze_news_sentiment(
            ticker=request.ticker,
            days=request.days
        )
...
def search_knowledge_base(
    query: str,
    collections: str = "kline,news,open_interest,analysis",
    top_k: int = 5
):
    """
    ...
    try:
        ...
        results = embedding_service.search(
            query=query,
            collection_types=collection_list,
            top_k=top_k
        )
...
@app.post("/api/vectordb/populate")
def trigger_populate(
    ...
    """
    Trigger population of vector database
    (This should ideally be a background task)
    
    Query params:
    - ticker: Ticker symbol
    - days: Number of days to fetch
    - include_kline: Include OHLCV data
    - include_news: Include news data
    - include_oi: Include open interest data
    """
    try:
        from data.populate_vectordb import (
            populate_kline_data,
            populate_news_data,
            populate_open_interest_data,
        ...
        if include_kline:
            populate_kline_data(ticker=ticker, days=days)
            results["populated"].append("kline")
        
        if include_news:
            populate_news_data(days=min(days, 30))
            results["populated"].append("news")
        
        if include_oi:
            populate_open_interest_data(symbol=ticker, days=days)
            results["populated"].append("open_interest")
        
        # Always add analysis examples
        populate_analysis_examples()
        results["populated"].append("analysis")
...
@app.post("/api/vectordb/update-incremental")
async def update_vectordb_incremental(background_tasks: BackgroundTasks):
    """Update vector DB với dữ liệu mới (chạy background)"""
    
    def update_task():
        from data.populate_vectordb import populate_kline_data, populate_news_data
        # Chỉ lấy 1 ngày gần nhất
        populate_kline_data(ticker="BTCUSDT", days=1)
        populate_news_data(days=1)
